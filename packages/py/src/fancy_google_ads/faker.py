...
def _customer_list_accessible(config: dict[str, Any], fake: FakeValues) -> Any:
    return {
        "resourceNames": [
...
def respond(operation: str, request: dict[str, Any]) -> Any:
    ...
    if operation == "customer_list_accessible":
        return _customer_list_accessible(config, fake)
    ...
    raise ValueError(
        f'google_ads: no fake response is defined for "{operation}". '
