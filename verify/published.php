...
/*
 * Google Ads — the published Composer package.
 *
...
if (! is_file($autoload)) {
    ...
    fwrite(STDERR, '    composer require particle-academy/google-ads-php'.PHP_EOL);
    exit(2);
}
...
foreach ($goldens as $golden) {
    ...
    $fake = new FakeValues(FakeValues::seedForCall('google_ads', $operation, $config));
    $faked = GoogleAdsFaker::respond($operation, ['config' => $config, 'fake' => $fake]);
