/*
 * Google Ads — the published npm packages.
 *
...
import { googleAdsFaker } from "@particle-academy/google-ads-js";
import { GOOGLE_ADS_KINDS } from "@particle-academy/google-ads-ui";
 ...
 */
const resolved = import.meta.resolve("@particle-academy/google-ads-js");
...
for (const { operation, config, expected } of GOLDENS) {
  const faked = googleAdsFaker(operation, fakeRequest("google_ads", operation, config));
...
assert.equal(GOOGLE_ADS_KINDS.length, 1);
for (const kind of GOOGLE_ADS_KINDS) {
...
console.log(`  ok   ui kinds resolve from ${"@particle-academy/google-ads-ui"}`);
