...
npm run provider -- google-ads            # regenerate this repo
npm run provider -- google-ads --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/google-ads-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/google-ads-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/google-ads-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-google-ads` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/google-ads/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/google-ads` |
| npm | `@particle-academy/google-ads-ui`, `@particle-academy/google-ads-js` |
| Packagist | `particle-academy/google-ads-php` |
| PyPI | `fancy-google-ads` |
...
**Default to plain HTTP.** A Google Ads SDK is third-party code and is subject to
